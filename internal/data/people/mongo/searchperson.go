...
func (s *searchPersonImpl) Execute(query queries.SearchPersonQuery) (queries.SearchPersonResult, error) {
	opts := options.Find().SetSort(bson.D{{Key: "id", Value: 1}}).SetLimit(int64(query.Limit))
	cursor, err := s.db.Collection("person").Find(s.ctx, bson.M{"id": bson.M{"$gt": query.LastID}}, opts)
	if err != nil {
